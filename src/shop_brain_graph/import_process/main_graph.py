...
from shop_brain_graph.import_process.nodes.node_entry import node_entry  # 入口节点：初始化参数、校验输入
from shop_brain_graph.import_process.nodes.node_pdf_to_md import node_pdf_to_md  # PDF转MD：解析PDF文件为markdown格式
from shop_brain_graph.import_process.nodes.node_md_img import node_md_img  # MD图片处理：提取/下载markdown中的图片、修复图片路径
from shop_brain_graph.import_process.nodes.node_document_split import node_document_split  # 文档分块：将长文档切分为符合模型要求的小片段
from shop_brain_graph.import_process.nodes.node_item_name_recognition import node_item_name_recognition  # 项目名识别：从分块中提取核心项目名称（业务定制化）
from shop_brain_graph.import_process.nodes.node_bge_embedding import node_bge_embedding  # BGE向量化：将文本分块转换为向量表示（适配Milvus向量库）
from shop_brain_graph.import_process.nodes.node_import_milvus import node_import_milvus  # 导入Milvus：将向量数据写入Milvus向量数据库
